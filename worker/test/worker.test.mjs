...
/* Everything chain-shaped comes from the real config, so this suite keeps
   passing when the placeholder addresses are swapped for a token's own. */
import { START_BLOCK, TOKENS, CONTRACTS } from '../src/config.js';
...
function makeEnv(head, transfers) {
  ...
  return { env: { REWARDS: KV, RPC_URL: 'http://rpc.test', ADMIN_TOKEN: 'secret' }, KV, rpcCalls };
}

const START = START_BLOCK;
...
test('fees track the rewards contract, not the platform-wide fee locker', async () => {
  const { STREAMS } = await import('../src/config.js');
...
test('holder payout strips the protocol cut', async () => {
  const { holderPayout, HOLDER_SHARE } = await import('../src/config.js');
  // Whatever HOLDER_SHARE is set to, the protocol's slice comes off the top.
  assert.equal(holderPayout({ paidOut: 100 }), 100 * HOLDER_SHARE);
...
test('counts holders from transfers, across a resumed backfill', async () => {
  ...
  for (const wnd of windows) {
    ...
    await runCron(mod, { REWARDS: KV, RPC_URL: 'http://rpc.test', ADMIN_TOKEN: 'secret' });
  }

  const env = { REWARDS: KV, RPC_URL: 'http://rpc.test', ADMIN_TOKEN: 'secret' };
  const dbg = await (await mod.fetch(new Request('https://w/debug'), env, ctx)).json();
